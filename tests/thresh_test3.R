...
pth1 <- paste0(root.pth,"7.gif") # Path to an image
pth2 <- paste0(root.pth,"2.gif") # Path to an image
...
# Overlap result, image coords:
dev.off()
par(mfrow=c(2,2))
plot(regA)         # Registered and (possibly) padded/chopped A
plot(B.img)        # B which A is registered to
plot(A.img)        # Original A
dev.off()

# Result, matrix coords:
plot(A.img[,,1])   # Original A
plot(regA[,,1])    # Registered and (possibly) padded/chopped A
plot(B.img[,,1])   # B which A is registered to

dev.off()
par(mfrow=c(1,2))
plot(flip(regA)[,,1], key=NULL)  # Registered and (possibly) padded/chopped A
plot(flip(B.img)[,,1], key=NULL) # B which A is registered to
