...
#'
#' @export
register.template.rch <- function(template.img, reference.img, h.min.overlap=1, w.min.overlap=1, method="SQDIFF", fill.typ="opposite", return.imgQ = F, printQ=F, plotQ=F){
  ...
  names(reg.info) <- c("shift.coords", "reg.params", "registered.template")
